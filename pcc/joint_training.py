...
def train_update(model, optimizer, data, update, settings, microbatch, *, mixed_precision=True):
    ...
    for offset in range(start, start + per_update, microbatch):
        ...
        with autocast(model, mixed_precision):
            sums, _ = model.losses(model(context), context)
...
@torch.no_grad()
def evaluate(model, data, microbatch, *, mixed_precision=True, rows=None):
    ...
    device = next(model.parameters()).device
    try:
        for start in range(0, limit, microbatch):
            context = data.batch(start, min(start + microbatch, limit), device)
            with autocast(model, mixed_precision):
...
def train(model, train_data, dev, output, identity, *, microbatch=1, resume=None,
          mixed_precision=True, stop_after=None):
    ...
    try:
        ...
        def monitor(step):
            sums, counts = evaluate(model, dev, microbatch, mixed_precision=mixed_precision,
                                    rows=settings.monitor_tokens // settings.context)
        ...
        if update == 0 and not history["validation"]:
            monitor(0)
        with (output / "train.jsonl").open("x") as log, (output / "validation.jsonl").open("x") as validation_log:
            ...
            for step in range(update + 1, target + 1):
                ...
                if step % settings.eval_every == 0 or step == target:
                    ...
                    save_checkpoint(output / "latest.pt", model, optimizer, identity, step, history)
                print(f"arm={model.arm} update={step}/{settings.updates} nll={record['nll']:.6f}", flush=True)
            if not (output / "latest.pt").exists():
                save_checkpoint(output / "latest.pt", model, optimizer, identity, target, history)
