#!/bin/bash
# Fixed-tokenizer Qwen adaptation of DeFINE with a low-dimensional tied head.
...
TASK_OUTPUT_DIR="$TASK_OUTPUT_BASE/define_tied_n112_k1724"
test -x "$TASK_PYTHON"
...
"$TASK_PYTHON" -m accelerate.commands.launch train_compositional.py \
    ...
    --run_name define-tied-n112-k1724-qwen3-0.6b \
    --arm define \
    --define_code_dim 112 \
    --define_expansion_dims 656,1184,1724 \
    --define_group_counts 16,8,4 \
    --tie_output
