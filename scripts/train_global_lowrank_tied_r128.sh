#!/bin/bash
# Explicitly named global low-rank tied systems baseline (existing LR128 math).
...
TASK_OUTPUT_DIR="$TASK_OUTPUT_BASE/global_lowrank_tied_r128"
test -x "$TASK_PYTHON"
...
"$TASK_PYTHON" -m accelerate.commands.launch train_compositional.py \
    ...
    --run_name global-lowrank-tied-r128-qwen3-0.6b \
    --arm global_lowrank \
    --d_x 128 \
    --tie_output
