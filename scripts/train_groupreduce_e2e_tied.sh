#!/bin/bash
# GroupReduce-style end-to-end tied adaptation at the LR128 parameter budget.
# Canonical post-hoc GroupReduce uses scripts/convert_dense_baseline.py instead.
...
TASK_OUTPUT_DIR="$TASK_OUTPUT_BASE/groupreduce_e2e_tied_g20"
test -x "$TASK_PYTHON"
...
"$TASK_PYTHON" -m accelerate.commands.launch train_compositional.py \
    ...
    --run_name groupreduce-e2e-tied-g20-qwen3-0.6b \
    --arm groupreduce \
    --groupreduce_num_groups 20 \
    --groupreduce_frequency_path resources/token_freq_sample10.npz \
    --groupreduce_frequency_key counts \
    --groupreduce_target_params 19579904 \
