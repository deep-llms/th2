"""One approved A1/A2 panel, then original all-eight-GPU burns; offline only."""
...
OUT = Path('/mnt/local/_outputs/deep-llms_th2/ccm_memory_diagnostics_seed17_20260914_a01')
PREVIOUS = Path('/mnt/local/_outputs/deep-llms_th2/ccm_pilot_seed29_20260913_a01')
STAGE2 = Path('/mnt/local/_outputs/deep-llms_th2/ccm_stage2_seed17_20260913_a01')
OLD_SESSION = 'ccm_seed29_common_20260913_a01'
...
def command(action, output):
    return [runtime.PYTHON, 'scripts/memory_diagnostics.py', action,
            '--data', str(runtime.DATA), '--stage2', str(STAGE2), '--output', str(output)]
...
def panel(smoke):
    ...
    check(not errors and len(children) == 3, f'Panel failed: {errors}')
    for arm in ARMS:
    ...
    print('THREE_DIAGNOSTIC_CHILDREN_VERIFIED_AND_REAPED', flush=True)
...
def observer():
    ...
    if not any(ident[2]):
        return None
    check(b'scripts/queue_seed29.py' in ident[2], 'Unexpected current observer; no takeover')
    return pid, ident
...
class Diagnostics(runtime.Workflow):
    ...
    def burns(self, indices):
        self.burn_number += 1
        name = f'ccm_memory_diag_20260914_a01_b{self.burn_number}'
        start(indices, name, OUT/f'{name}.log', 30140+self.burn_number)
    ...
    def disarm(self):
        ...
        check(complete.get('success') is True and complete.get('event') == 'seed29_common_verified_and_burns_active'
              and status.get('success') is True and status['stage'] == 'complete' and -10 <= age <= 180,
              'Seed-29 completion/heartbeat not valid; burns untouched')
        observed = observer()
    ...
    def execute(self):
        ...
        self.free_after_wait(ALL)
        self.burns(SPARE)
        self.run('smoke', ['/usr/bin/python3', 'scripts/pilot_memory_diagnostics.py', 'smoke'], ACTIVE, SPARE)
        require_free(ACTIVE)
        self.run('full_dev', ['/usr/bin/python3', 'scripts/pilot_memory_diagnostics.py', 'full'], ACTIVE, SPARE)
        ...
        inspect(ALL, active=True)
        self.stage = 'complete'
        self.record(event='memory_diagnostics_verified_and_burns_active', success=True, gpus=snapshot(ALL))
