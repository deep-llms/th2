"""Authorized seed-17 Stage-2 matched continuation; no locked-val or replication launch."""
import json
import os
...
OUT = Path('/mnt/local/_outputs/deep-llms_th2/ccm_stage2_seed17_20260913_a01')
ARMS = ('base', 'contextual', 'isolated', 'shuffled', 'grad')
PREVIOUS = Path('/mnt/local/_outputs/deep-llms_th2/ccm_stage1_seed17_20260913_a01')
OLD_SESSION = 'ccm_stage1_20260913_a01'


def train_command(arm):
    check(arm in ARMS, 'Unknown Stage-2 arm')
    cmd = [PYTHON, '-m', 'torch.distributed.run', '--standalone', '--nproc_per_node=8',
           '--module', 'ccm', 'train', '--phase', 'stage2', '--arm', arm, '--seed', '17',
...
def eval_command(arm):
    check(arm in ARMS, 'Unknown evaluation arm')
    ck = OUT/'train'/arm/'checkpoint-3815'
...
class Stage2(Workflow):
    ...
    def burns(self, indices):
        self.burn_number += 1
        label = f'ccm_stage2_20260913_a01_b{self.burn_number}'
        start(indices, label, OUT/f'{label}.log', 29940+self.burn_number)
    ...
    def validate(self, stage, arm=None, spare=()):
        cmd = [PYTHON, 'scripts/validate_stage2.py', stage, '--workflow', str(OUT)]
        if arm:
            cmd += ['--arm', arm]
    ...
    def disarm_old_observer(self):
        ...
        if exists:
            ...
            if info[0] == '0':
                ...
                check(b'scripts/pilot_stage1.py' in original[2], 'Unexpected previous observer process')
    ...
    def execute(self):
        ...
        check(not OUT.exists() and not OUT.is_symlink(), 'Stage-2 output must be fresh')
        OUT.mkdir()
        ...
        inspect(ALL, active=True)
        self.stage = 'complete'
        self.record(event='stage2_five_arms_dev_evaluated_and_burns_active', success=True, gpus=snapshot(ALL))
