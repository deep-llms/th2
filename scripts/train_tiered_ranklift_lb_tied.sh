#!/bin/bash
# Tiered RankLift: language-balanced GroupReduce allocation plus tail/torso lifts.
...
TASK_OUTPUT_DIR="$TASK_OUTPUT_BASE/tiered_ranklift_lb_t4_c512"
TASK_IMPORTANCE="resources/token_importance_langbalanced.npz"
test -x "$TASK_PYTHON"
test -s "$TASK_IMPORTANCE"
test -s "${TASK_IMPORTANCE%.npz}.json"
...
"$TASK_PYTHON" -m accelerate.commands.launch train_compositional.py \
    ...
    --run_name tiered-ranklift-lb-t4-c512-qwen3-0.6b \
    --arm tiered_ranklift \
    ...
    --tiered_ranklift_frequency_path "$TASK_IMPORTANCE" \
    --tiered_ranklift_frequency_key counts \
    --tiered_ranklift_rms_eps 1e-6 \
    --tie_output
