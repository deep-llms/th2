#!/bin/bash
# BT-MoS primary: language-balanced GroupReduce + exact-tied K-way MoS head.
...
TASK_OUTPUT_DIR="${BTMOS_OUTPUT_DIR:-$TASK_OUTPUT_BASE/btmos_k3_c256_lb}"
IMPORTANCE_PATH="${BTMOS_IMPORTANCE_PATH:-resources/token_importance_langbalanced.npz}"
GROUP_RANKS="${BTMOS_GROUP_RANKS:-1024,352,192,64}"
RUN_NAME="${BTMOS_RUN_NAME:-btmos-k3-c256-lb-qwen3-0.6b}"
test -x "$TASK_PYTHON"
test -s "$IMPORTANCE_PATH"
...
"$TASK_PYTHON" -m accelerate.commands.launch train_compositional.py \
    ...
    --groupreduce_ranks "$GROUP_RANKS" \
    --groupreduce_populations 2048,6144,24576,119168 \
    --groupreduce_frequency_path "$IMPORTANCE_PATH" \
