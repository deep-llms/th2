#!/bin/bash
# Unified RankLift: variable private codes, raw-frequency allocation, one tied output.
...
TASK_OUTPUT_DIR="$TASK_OUTPUT_BASE/unified_ranklift_raw_t4_m460"
TASK_IMPORTANCE="resources/token_freq_sample10.npz"
test -x "$TASK_PYTHON"
...
"$TASK_PYTHON" -m accelerate.commands.launch train_compositional.py \
    ...
    --run_name unified-ranklift-raw-t4-m460-qwen3-0.6b \
    ...
    --unified_ranklift_frequency_path "$TASK_IMPORTANCE" \
    --unified_ranklift_frequency_key counts \
    --unified_ranklift_rms_eps 1e-6 \
    --tie_output
