#!/bin/bash
# Phase-1 Nested Ladder: frequency-nested additive ranks with exact tying.
...
TASK_OUTPUT_DIR="$TASK_OUTPUT_BASE/nested_ladder_tied_t4"
test -x "$TASK_PYTHON"
...
"$TASK_PYTHON" -m accelerate.commands.launch train_compositional.py \
    ...
    --run_name nested-ladder-tied-t4-qwen3-0.6b \
    --arm nested_ladder \
    --nested_tier_ranks 64,128,320,512 \
    --nested_tier_populations 151936,32768,8192,2048 \
    --nested_frequency_path resources/token_freq_sample10.npz \
    --nested_frequency_key counts \
    --tie_output
