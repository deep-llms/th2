#!/bin/bash
# B200-matched global rank-128 factorization with exact input/output tying.
...
TASK_OUTPUT_DIR="$TASK_OUTPUT_BASE/global_lowrank_tied_r128_b200"
test -x "$TASK_PYTHON"
...
"$TASK_PYTHON" -m accelerate.commands.launch train_compositional.py \
    ...
    --run_name global-lowrank-tied-r128-b200-qwen3-0.6b \
    --arm lowrank \
    --d_x 128 \
    --tie_output
