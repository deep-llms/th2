#!/bin/bash
# Slim tied adaptation, exactly parameter-matched to global low-rank r=128.
...
TASK_OUTPUT_DIR="$TASK_OUTPUT_BASE/slim_tied_k4_m76484"
test -x "$TASK_PYTHON"
...
"$TASK_PYTHON" -m accelerate.commands.launch train_compositional.py \
    ...
    --run_name slim-tied-k4-m76484-qwen3-0.6b \
    --arm slim \
    --slim_num_components 4 \
    --slim_num_subvectors 76484 \
    --slim_mapping_seed 42 \
    --tie_output
