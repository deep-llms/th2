#!/bin/bash
# Published P-VQ phase C: compact exact-tied fine-tuning.

set -euo pipefail

: "${PVQ_CURRICULUM_CHECKPOINT:?Set PVQ_CURRICULUM_CHECKPOINT to phase-B checkpoint}"
: "${PVQ_INIT_PATH:?Set PVQ_INIT_PATH to converted compact embedding state}"
...
TASK_OUTPUT_DIR="$TASK_OUTPUT_BASE/pvq_tied_w768_k128"
test -x "$TASK_PYTHON"
test -d "$PVQ_CURRICULUM_CHECKPOINT"
test -s "$PVQ_INIT_PATH"
...
"$TASK_PYTHON" -m accelerate.commands.launch train_compositional.py \
    --model_name_or_path "$PVQ_CURRICULUM_CHECKPOINT" \
    ...
    --run_name pvq-tied-w768-k128-qwen3-0.6b \
    --arm pvq \
    --pvq_shared_dim 768 \
    --pvq_num_codes 128 \
    --embedding_init_path "$PVQ_INIT_PATH" \
    --tie_output
