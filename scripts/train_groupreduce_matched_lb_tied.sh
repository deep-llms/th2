#!/bin/bash
# Ordering-only control: matched GroupReduce ranks with language-balanced tiers.
...
TASK_OUTPUT_DIR="${BTMOS_OUTPUT_DIR:-$TASK_OUTPUT_BASE/groupreduce_matched_lb_t4}"
...
"$TASK_PYTHON" -m accelerate.commands.launch train_compositional.py \
    ...
    --run_name groupreduce-matched-lb-t4-qwen3-0.6b \
    --arm groupreduce \
