...
TASK_SESSION=ccm_stage2_20260913_a01
TASK_OUTPUT=/mnt/local/_outputs/deep-llms_th2/ccm_stage2_seed17_20260913_a01
TASK_LOG=/mnt/local/_outputs/deep-llms_th2/ccm_stage2_seed17_20260913_a01.handoff.log
test ! -e "$TASK_OUTPUT" && test ! -L "$TASK_OUTPUT"
test ! -e "$TASK_LOG" && test ! -L "$TASK_LOG"
if tmux has-session -t "$TASK_SESSION" 2>/dev/null; then
    echo 'REFUSE: Stage-2 session already exists' >&2
    exit 1
fi
/usr/bin/python3 scripts/pilot_gpu_ops.py inspect --gpus 0 1 2 3 4 5 6 7
printf -v TASK_CMD 'cd /mnt/local/deep-llms_th2 && source /mnt/local/conda-py311/etc/profile.d/conda.sh && conda activate train_env && exec /usr/bin/python3 -u scripts/pilot_stage2.py >%q 2>&1' "$TASK_LOG"
...
print('CCM_STAGE2_HANDOFF_ARMED', s)
PY
