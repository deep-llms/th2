#!/usr/bin/env bash
# Evaluate raw Tiered and Unified RankLift at step 10000,
# using the standard
...
start_burn() {
    ...
    mkdir -p "$(dirname "$log")"
    # Own terminal/session: the burn survives completion of the runner shell.
    # Preserve dead panes and exit status so later failures can be diagnosed.
    local session="burn_${TASK_RUN_ID}_${port}"
    ...
    local launcher
    launcher="$(tmux display-message -p -t "$session" '#{pane_pid}')"
...
TASK_TIERED="$TASK_OUTPUT_BASE/tiered_ranklift_raw_t4_c512/checkpoint-10000"
...
restore_burn_on_failure() {
    ...
    if [[ "$status" -ne 0 && "$TASK_FINAL_BURN_STARTED" -ne 1 ]]; then
        ...
        if [[ "${#TASK_FAILURE_GPU_PIDS[@]}" -eq 0 ]]; then
            ...
            echo "Recovery burn persists independently in tmux."
        else
...
command -v tmux >/dev/null || die 'tmux is required for persistent burns'
validate_b200_node
...
expected = {
    checkpoints[0]: ('tiered_ranklift', 10000, {
        ...
        'tiered_ranklift_frequency_key': 'counts',
    }),
    checkpoints[1]: ('unified_ranklift', 10000, {
        'unified_ranklift_code_dims': '384,256,128,112',
        'unified_ranklift_feature_dim': 460,
        'unified_ranklift_populations': '2048,6144,24576,119168',
        'unified_ranklift_frequency_key': 'counts',
    }),
}
...
grep -Fq 'Loaded compositional model: arm=unified_ranklift' "$TASK_UNIFIED/eval.log"
scan_fatal_logs evaluation "$TASK_EVAL_LAUNCH_LOG" "$TASK_TIERED/eval.log" "$TASK_UNIFIED/eval.log"
...
for TASK_LOG in "$TASK_FINETUNE_OUTPUT"/*tiered_ranklift_raw_c512_s10000*.log; do
...
for TASK_LOG in "$TASK_FINETUNE_OUTPUT"/*unified_ranklift_raw_m460_s10000*.log; do
    grep -Fq 'Loaded compositional model: arm=unified_ranklift' "$TASK_LOG" \
        || die "Unified RankLift loader confirmation missing from $TASK_LOG"
done
...
# The final burn is owned by its separate tmux session, not this shell.
# Check sustained progress before finishing the finite workflow.
sleep 30
[[ "$(burn_launcher_for_indices 0,1,2,3,4,5,6,7 8)" == "$TASK_FINAL_BURN_LAUNCHER" ]] \
    || die 'final burn ownership changed during sustained check'
tail -n 5 "$TASK_FULL_BURN_LOG"
echo "FINAL_BURN_PERSISTS_IN_TMUX launcher=$TASK_FINAL_BURN_LAUNCHER"
