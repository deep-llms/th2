#1 +60+a
#th2-swt-six-checkpoint-eval-finetune-smoke-20260909-a01
set -euo pipefail
...
TASK_ROOT=/mnt/local/_outputs/deep-llms_th2/swt/eval_finetune_smoke_20260909_a01
test ! -e "$TASK_ROOT"
mkdir -p "$TASK_ROOT"
...
sleep 30
python scripts/gpu_status.py --gpus 0 1 2 3 4 5 6 7 --require-free
TASK_PRECISION_EXIT=0
CUDA_VISIBLE_DEVICES=0 python -u -m scripts.check_eval_precision --device cuda --output "$TASK_ROOT/precision.json" > "$TASK_ROOT/precision.log" 2>&1 || TASK_PRECISION_EXIT=$?
echo PRECISION_TEST_EXIT="$TASK_PRECISION_EXIT"
...
for arm in ('B0','A128','A256','A512','C','D'):
    ...
    reports[arm]={'success':result['success'],'source_checkpoint_unchanged':True,
