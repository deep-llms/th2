#1 +60+a
#th2-verify-culturax-and-tokenizer-fresh-b200-20260828-a01
set -euo pipefail
...
echo '=== tokenizer/model availability for offline sampling ==='
if [ -s "$TASK_MODEL_DIR/tokenizer.json" ] && [ -s "$TASK_MODEL_DIR/tokenizer_config.json" ]; then
    echo "tokenizer_model_ready=YES path=$TASK_MODEL_DIR"
else
    echo "tokenizer_model_ready=NO path=$TASK_MODEL_DIR"
fi
