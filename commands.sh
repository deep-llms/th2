...
TASK_LOG_DIR=/mnt/local/_outputs/@PROJECT@/data_preparation/culturax_qwen_78gg_20260926_a01
test ! -e "$TASK_LOG_DIR"
...
/mnt/local/conda-py311/envs/train_env/bin/python -B - <<'PY_RECEIPT'
import hashlib,json
from pathlib import Path
receipt=json.loads(Path('/mnt/local/_outputs/deep-llms_th2/data_preparation/culturax_78gg_preflight_20260926_a01.json').read_text())
assert receipt['status']=='ok' and receipt['raw_hashes_verified'] and receipt['files']==75
assert receipt['manifest_sha256']==hashlib.sha256(Path('resources/culturax_raw_manifest.tsv').read_bytes()).hexdigest()
assert receipt['prepare_data_sha256']==hashlib.sha256(Path('prepare_data.py').read_bytes()).hexdigest()
print('RAW_DATA_PREFLIGHT_RECEIPT_VERIFIED',flush=True)
PY_RECEIPT
