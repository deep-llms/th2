#1 +30+a
#th2-swt-readonly-final-completion-and-burn-20260911-a01
set -euo pipefail
date -u
hostname
...
for sample in range(2):
    print('LIVE_GPU_SNAPSHOT', sample, json.dumps(snapshot()), flush=True)
    try:
        workers, launcher=verified_workers('/tmp/llm_pretrain_burn.py',list(range(8)))
        print('VERIFIED_BURN_WORKERS', json.dumps(workers), 'LAUNCHER', json.dumps(launcher), flush=True)
    except Exception as exc:
        print('BURN_OWNERSHIP_NOT_CONFIRMED', type(exc).__name__, str(exc), flush=True)
    if log.exists():
        with log.open('rb') as handle:
            handle.seek(max(0,log.stat().st_size-14000))
            lines=handle.read().decode(errors='replace').splitlines()
        print('BURN_LOG_TAIL', '\n'.join(lines[-14:]), flush=True)
    else:
        print('BURN_LOG_ABSENT', flush=True)
    if sample==0:
        time.sleep(15)
PY
tmux list-panes -a -F '#{session_name} #{pane_pid} #{pane_dead} #{pane_current_command}' || true
date -u
