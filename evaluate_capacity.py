...
def main():
    ...
    data = PackedTokens(args.data, args.split, verify_hash=True)
    model = AutoModelForCausalLM.from_pretrained(args.checkpoint, local_files_only=True).to(args.device).eval()
    if (model.config.vocab_size != data.manifest["vocab_size"] or
            model.config.eos_token_id != data.manifest["eos_token_id"]):
    ...
    with torch.no_grad():
        for batch in DataLoader(data, batch_size=args.batch_size, shuffle=False):
            ...
            nll_sum += loss * count
            target_count += count
    if target_count != len(data) * (data.length - 1):
    ...
    result = dict(success=True, split=args.split, nll=nll, ppl=math.exp(nll) if nll < 700 else None,
                  scored_targets=target_count, processed_tokens=len(data)*data.length,
